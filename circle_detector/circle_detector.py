...
class CircleDetectorNode(Node):
    def __init__(self):
        ...
        self.VELOCITY_XY_P_GAIN = 3.0  # Higher proportional gain for horizontal control (increased from 1.5)
        self.VELOCITY_Z_P_GAIN = 0.5  # Original gain for altitude control
        ...
        # Flag to indicate if we should ignore mode service errors
        self.ignore_mode_errors = True
    ...
    def control_loop(self):
        ...
        try:
            ...
            self.get_logger().info(f'Drone position: x={self.current_pose.pose.position.x:.2f}, y={self.current_pose.pose.position.y:.2f}, z={self.current_pose.pose.position.z:.2f}')
            self.get_logger().info(f'Target position: x={self.target_pose.pose.position.x:.2f}, y={self.target_pose.pose.position.y:.2f}, z={self.target_pose.pose.position.z:.2f}')
            
            # The target position is in the camera frame, which might be different from the drone's body frame
            # For simplicity, let's use the target position directly, which assumes camera frame = drone body frame
            # This is what we're already doing, but let's be explicit about the assumption
            
            # Calculate error between current position and target
            # Note: This assumes target_pose is already in the correct frame relative to the drone
            ...
            # Add info logging for position tracking (upgraded from debug to info for better visibility)
            self.get_logger().info(f'Target error: dx={dx:.2f}, dy={dy:.2f}, dist={distance:.2f}')
            ...
            vx = float(self.VELOCITY_XY_P_GAIN * dx)
            vy = float(self.VELOCITY_XY_P_GAIN * dy)
            vz = float(self.VELOCITY_Z_P_GAIN * dz)
            ...
            # Add info logging for velocity values
            self.get_logger().info(f'Velocity commands: vx={vx:.2f}, vy={vy:.2f}, vz={vz:.2f}')
            
            # Calculate yaw to face target
            #yaw = float(math.atan2(dy, dx))
            yaw = 0.0  # Fixed yaw - prevent rotation while navigating
            
            # Force minimum velocity if the value is very small but not zero
            # This helps overcome any potential deadband in the control system
            min_effective_vel = 0.1  # m/s
            if 0 < abs(vx) < min_effective_vel:
                vx = min_effective_vel * (1.0 if vx > 0 else -1.0)
            if 0 < abs(vy) < min_effective_vel:
                vy = min_effective_vel * (1.0 if vy > 0 else -1.0)
    ...
    def publish_velocity(self, vx, vy, vz, yaw):
        ...
        # Log the actual values being sent (after any conversion)
        self.get_logger().info(f'Publishing Joy velocity: vx={joy_msg.axes[0]:.2f}, vy={joy_msg.axes[1]:.2f}, vz={joy_msg.axes[2]:.2f}, yaw={joy_msg.axes[3]:.2f}')
        ...
        try:
            ...
            if circles is not None:
                ...
                if not was_target_detected:
                    ...
                    # Switch to LOITER mode when circle is first detected and we're not already in circle approach mode
                    if not self.in_circle_approach_mode and not self.drop_completed:
                        ...
                        if self.dds_connected:
                            self.get_logger().info('Circle detected! Switching to LOITER mode for approach')
                            if self.set_mode(self.MODE_LOITER):
                                self.in_circle_approach_mode = True
                                self.drop_completed = False
                        else:
                            self.get_logger().warn('Circle detected but DDS not connected. Cannot switch to LOITER mode.')
            ...
            if self.dds_connected:
                mode_text = f"Mode: {'LOITER' if self.in_circle_approach_mode else 'AUTO'}"
    ...
    def mode_change_callback(self, future, requested_mode):
        ...
        try:
            ...
            if response.status:
                ...
                if requested_mode == self.MODE_LOITER:
                    self.in_circle_approach_mode = True
                    self.drop_completed = False
